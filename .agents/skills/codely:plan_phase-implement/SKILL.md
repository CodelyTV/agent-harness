...
5. **STOP.** Tell the user to review the changes and **suggest 3 alternative commit messages**. Follow the commit message convention defined in the [`commit-messages.md`](../codely:git-commit/resources/commit-messages.md) file. Use different plausible commit message types in the 3 alternatives. Specify the workspace as scope if only one workspace is affected. In the suggestion, make it simple to reply something like "commit with message 1", and use the [`/codely:git-commit`](../codely:git-commit/SKILL.md) skill to commit the changes specifying the chosen message.
